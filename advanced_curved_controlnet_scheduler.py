...
class AdvancedCurvedControlNetScheduler:
    ...
    def validate_inputs(self, start_percent, end_percent, num_keyframes, start_strength, end_strength):
        ...
        if start_strength < 0 or end_strength < 0:
            warnings.append(f"Negative strength values detected")
        
        if start_strength == end_strength:
            warnings.append(f"Start and end strength are identical - curve will be flat")
...
============================"""
    ...
    def save_curve_to_csv(self, percents, strengths, filename):
        ...
        try:
            output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
                os.path.abspath(__file__)))), "output", "curves")
    ...
    def generate_keyframes(self, preset, mode, num_keyframes, start_percent, end_percent, 
                          ...
                          comparison_curve="none", save_curve=False, curve_filename="curve_export"):
        ...
        try:
            ...
            if mode == "steps":
                start_percent = start_percent if start_percent <= 1.0 else start_percent / total_steps
                end_percent = end_percent if end_percent <= 1.0 else end_percent / total_steps
            ...
            for i, (percent, strength) in enumerate(zip(percents, strengths)):
                ...
                if print_keyframes:
                    print(f"[Advanced Curved] KF {i}: {percent:.4f}, {strength:.4f}")
    ...
    def generate_graph(self, percents, strengths, curve_type, curve_param, 
        ...
        try:
            ...
            if comparison_data is not None:
                ...
                ax.plot(comp_percents * 100, comp_strengths, 'g--', linewidth=2, 
                       label=f'Compare: {comp_name}', alpha=0.6)
